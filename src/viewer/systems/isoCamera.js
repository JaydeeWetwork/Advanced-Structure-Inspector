 ...
 */
/**
 * Perspective distance that matches an ortho half-height at `fovDeg`.
 * @param {number} halfHeight
 * @param {number} [fovDeg]
 */
export function perspectiveDistanceFromOrthoHalfHeight(halfHeight, fovDeg = 70) {
	const hh = Math.max(Number(halfHeight) || 1, 1);
	const fov = Number(fovDeg);
	const rad = (((Number.isFinite(fov) ? fov : 70) * Math.PI) / 180);
	const t = Math.tan(rad / 2);
	if (!(t > 1e-6)) return hh;
	return Math.max(hh / t, 16);
}
