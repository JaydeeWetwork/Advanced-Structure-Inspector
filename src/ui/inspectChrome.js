...
import { formatInspectLocation } from "../viewer/inspectPos.js";
	...
	try {
		...
		if (hit.kind === "miss" || !isContainer) {
			if (hit.kind === "block" && hit.block) {
				...
				wrap.innerHTML = `<div class="mc-inv-header"><span class="mc-inv-title">${escapeHtml(hit.block.name)}</span><button type="button" class="mc-inv-close" data-action="close-inspect" aria-label="Close">×</button></div>${loc ? `<div class="mc-inv-location">${escapeHtml(loc)}</div>` : ""}`;
				...
				const loc = formatInspectLocation(hit);
				wrap.innerHTML = `<div class="mc-inv-header"><span class="mc-inv-title">${label}</span><button type="button" class="mc-inv-close" data-action="close-inspect" aria-label="Close">×</button></div>${loc ? `<div class="mc-inv-location">${escapeHtml(loc)}</div>` : ""}`;
				showInspectPanelNode(wrap);
			} else {
				clearInspectPanel();
			}
